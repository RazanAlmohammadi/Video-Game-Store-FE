...
export default function UserManagementDashboard() {
    ...
    const fetchUsers = async (page) => {
        ...
        try {
            setLoading(true);
            const response = await axios.get(`  https://sda-3-online-backend-teamwork-ec29.onrender.com/api/v1/Customer`, {
                params: { offset, limit },
                headers: {
    ...
    const deleteUser = async (personId) => {
        ...
        try {
            const response = await axios.delete(`  https://sda-3-online-backend-teamwork-ec29.onrender.com/api/v1/SystemAdmin/${personId}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
